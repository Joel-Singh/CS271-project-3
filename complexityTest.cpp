...
int test_complexity() {
    ...
    for (int size: sizes) {
        ...
        TIME_TEST(ht.insert(rand(), rand()), "Randomized Hashtable insert with "
                  + to_string(element_count) + " elements " + "of size " +
                  to_string(size));

        cout << "The highest load is " << (float) ht.get_highest_load() /
            ((float) element_count / (float) size) << " times the ideal load"
            << endl << endl;
    }
...
//
// RETURN VALUE:
//  Returns a int hashtable with randomized elements of key and data being any possible int.
//=================================================
HashTable<int> create_randomized_hashtable(int size, int element_count) {
