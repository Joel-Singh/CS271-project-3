...
void test_get_key() {
  try {
    ...
    if (empty_elem.get_key() != -1) {
      cout << "Incorrect result from get key. Expected -1 for an empty element "
              "but got : "
           << empty_elem.get_key() << endl;
    }
    Element<int> e(10, 6);
    if (e.get_key() != 6) {
      cout << "Incorrect result from get key. Expected 6 but got : "
           << e.get_key() << endl;
    }
  } catch (exception &e) {
...
void test_get_data() {
  try {
    ...
    if (empty_elem.get_data() != 0) {
      cout << "Incorrect result from get data. Expected 0 for an empty element "
              "but got : "
           << empty_elem.get_data() << endl;
    }
    Element<int> e(10, 6);
    if (e.get_data() != 10) {
      cout << "Incorrect result from get data. Expected 10 but got : "
           << e.get_data() << endl;
    }
  } catch (exception &e) {
...
void test_remove() {
  ...
  try {
    ...
    ht.remove(4, 5);
    if (ht.to_string() != "0: \n1: (10,6) \n2: \n3: \n4: \n") {
      cout << "Incorrect result of removing non-member from table. "
              "Expected\n\n0: \n1: (10,6) \n2: \n3: \n4: \n\nBut got\n\n"
           << ht.to_string() << endl;
    }
  } catch (exception &e) {
...
void test_member() {
  try {
    ...
    cout << "Error caused by trying to determine membership in empty table : "
         << e.what() << endl;
  }
  try {
    HashTable<int> ht(5);
...
void test_login() {
  try {
    ...
    password = "98213873";
    if (login(ht, username, password)) {
      cout << "Incorrect result validating inaccurate login information"
           << endl;
    }
  } catch (exception &e) {
...
// These tests assume using USE_K_MOD_M (e.g a key of value 3 will be in index
// 3)
#ifdef USE_K_MOD_M
  test_get_key();
  test_get_data();
